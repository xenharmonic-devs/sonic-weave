...
describe('SonicWeave parser', () => {
  ...
  it('can construct well-temperaments (manual)', () => {
    const scale = parseSource(`
      ...
      cents;
    `);
    const cents = scale.map(i => i.value.totalCents());
    expect(cents).toHaveLength(12);
    const expected = [
      91.52832958332323, 196.08999826922548, 294.1349974038386,
      392.1799965384506, 498.0449991346128, 590.876662628194, 698.0449991346126,
      792.1799965384507, 894.1349974038386, 996.0899982692256,
      1090.224995673063, 1200,
    ];
    expected.forEach((value, i) => {
      expect(cents[i]).toBeCloseTo(value, 12);
    });
  });
