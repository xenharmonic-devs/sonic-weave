...
riff TE(valsOrCommas, basis=niente) {
  "Create a Tenney-Euclid optimal temperament from an array of vals or commas. Note: Comma lists will always use the full inferred prime limit, but a different basis may be given."
...
riff CTE(valsOrCommas, primeLimit=niente) {
  "Create a constrained (nearly) Tenney-Euclid optimal temperament from an array of vals or commas. Note: Comma lists will always use the full inferred prime limit, but a higher one can be given."
  if (not isArray(valsOrCommas)) {
    valsOrCommas = [valsOrCommas];
  }
...
riff POTE(valsOrCommas, primeLimit=niente) {
  "Create a naïve pure-equaves tuning by normalizing the Tenney-Euclid optimal temperament based on an array of vals or commas. Note: Comma lists will always use the full inferred prime limit, but a higher one can be given."
  if (not isArray(valsOrCommas)) {
    valsOrCommas = [valsOrCommas];
  }
  if (isVal(valsOrCommas[0])) {
